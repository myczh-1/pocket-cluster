...
- **可恢复回收站** — 在保留期内恢复已删除文件或完整目录树
- **版本历史** — 恢复近期被替换的文件内容，同时保留原有版本谱系
- **副本就绪状态** — 在文件列表直接确认文件仍在复制，还是已经可以安全关闭设备
- **设备安全退出** — 永久停用设备前，先把数据迁移到其他设备并完成校验
...
从 [Releases](https://github.com/myczh-1/pocket-cluster/releases) 下载对应平台的二进制文件：
...
4. **等待副本就绪** — 文件显示“副本已就绪”后，再关闭或带走源设备
...
- **三节点离线恢复** — 节点离线期间删除并从回收站恢复目录，验证节点重新上线后收敛（`three-node-offline-restore.sh`）
- **三节点版本恢复** — 节点离线期间恢复旧文件内容，验证节点重新上线后收敛（`three-node-offline-version-restore.sh`）
- **WebDAV 冒烟** — 上传、列表、下载、删除（`webdav-smoke-test.sh`）
- **Android 手动** — 入池与携带流程清单（`android-manual-test.md`）
- **Android 连接冒烟** — 自动安装、启动、端口转发并验证真机健康状态（`android-connected-smoke.sh`）
