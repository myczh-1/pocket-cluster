...
`file_id` is generated by the creating node and shared with every peer. Receivers still accept older events without it.
...
Peers restore directory trees by path so tombstones created by releases with node-local directory IDs remain recoverable.
