...
### 11. Local File Browser And Move To Pool

- User story: 作为用户，我希望先查看当前节点本机磁盘上的文件，再选择其中一部分迁移进资源池释放当前设备空间。
- Description: 这是独立于普通上传的本节点本地文件管理工作流。Agent 需要列出当前节点可访问的本地文件；用户从本地文件列表中选择文件执行迁移；系统确认资源池写入、副本生成和元数据同步完成后，才进入删除本地原文件或用户确认删除流程。普通文件上传不触发本地删除。
- Acceptance criteria:
  - [ ] 用户可以浏览当前节点本机文件列表。
  - [ ] 用户可以从本机文件列表选择文件执行迁移到资源池。
  - [ ] 迁移前系统完成资源池写入和副本生成。
  - [ ] 迁移成功后，本地原文件被保留、移除，或进入用户确认的删除流程；具体策略必须由该迁移工作流显式决定。
  - [ ] 迁移失败时本地原文件不丢失。
  - [ ] 普通上传流程不会删除或移动本地原文件。
...
### 17. Syncthing-Style Conflict Handling
