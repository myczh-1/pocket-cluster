...
run_case() {
  ...
  local base_etag
  base_etag="$(e2e_webdav_etag 0 "/docs/report.txt")"
  [[ -n "${base_etag}" ]] || { echo "[${case_name}] missing base ETag" >&2; return 1; }

  # Each writer sees only v1: stop its peers before the conditional overwrite.
  e2e_stop_node 1
  e2e_stop_node 2
  ...
  # Node C is an unchanged observer; restore writers in the requested order.
  e2e_start_node 2
  e2e_login_node 2
  ...
  e2e_webdav_put 2 "/docs/report.txt" "${after_file}" "$(e2e_webdav_etag 2 "/docs/report.txt")"
  local continued
